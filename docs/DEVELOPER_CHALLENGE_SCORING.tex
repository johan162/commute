...
% Code listings for source inclusion (TypeScript with line numbers)
\IfFileExists{listings.sty}{%
  \usepackage{listings}
  % load the predefined language (case-sensitive name)
  % \lstloadlanguages{JavaScript}
  \lstdefinelanguage{JavaScript}{
  sensitive=true,
  morekeywords={break,case,catch,continue,debugger,default,delete,do,else,false,finally,for,function,if,in,instanceof,new,null,return,switch,this,throw,true,try,typeof,var,let,const,void,while,with,yield,async,await,import,export,from,as},
  morecomment=[l]{//},
  morecomment=[s]{/*}{*/},
  morestring=[b]",
  morestring=[b]'
  ...
  \usepackage{courier}
  % Define a TypeScript-friendly style based on JavaScript
  \lstdefinestyle{tsstyle}{%
    language=JavaScript,
    morekeywords={interface,type,namespace,readonly,public,private,protected,abstract,implements,extends,as},
...
\item
  ...
  bound and \(H\) the upper bound (in minutes)
\item
...
% Appendix with source code listing
\clearpage
\appendix
\section{Appendix: Source code (scoring.ts)}\label{appendix:scoring}
\noindent The following listing includes an example implementation of the scoring function using typescript.  
Line numbers are shown for reference.

\vspace{6pt}
\lstinputlisting[style=tsstyle,caption={scoring.ts — source listing},label={lst:scoring}]{scoring.ts}
